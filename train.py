...
from hi_sam.data.dataloader import (
    create_dataloaders,
    custom_collate_fn,
    eval_transforms,
    get_im_gt_name_dict,
...
from hi_sam.modeling.loss import (
    loss_hi_iou_mse,
    loss_hi_masks,
    loss_iou_mse,
    loss_masks,
...
def get_args_parser():
    ...
    parser.add_argument(
        "--train_datasets", type=str, nargs="+", default=["totaltext_train"]
    )
    parser.add_argument(
        "--val_datasets", type=str, nargs="+", default=["totaltext_test"]
    )
    parser.add_argument(
        "--hier_det",
        action="store_true",
    ...
    parser.add_argument(
        "--lr_charac_mask_decoder_name", default=["mask_decoder"], type=str, nargs="+"
    )
...
def main(train_datasets, valid_datasets, args):
    ...
    if not args.eval:
        ...
        param_dicts = [
            {
                "params": [
                    ...
                    if not match_name_keywords(n, args.lr_charac_mask_decoder_name)
                    and p.requires_grad
                ],
                "lr": args.lr,
            },
            {
                "params": [
                    p
                    for n, p in model_without_ddp.named_parameters()
                    if match_name_keywords(n, args.lr_charac_mask_decoder_name)
                    and p.requires_grad
                ],
        ...
        optimizer = optim.AdamW(
            param_dicts, lr=args.lr, betas=(0.9, 0.999), weight_decay=0.05
        )
...
def train(
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        for data in metric_logger.log_every(train_dataloaders, 50):
            inputs, labels = (
                data["image"],
                data["label"].to(model.device),
            )  # (bs,3,1024,1024), (bs,1,1024,1024)
            batched_input = []
            if args.hier_det:
                para_masks, line_masks, word_masks = (
                    data["paragraph_masks"],
                    data["line_masks"],
                    data["word_masks"],
                )
                line2para_idx = data["line2paragraph_index"]
                fg_points, para_masks, line_masks, word_masks = (
                    misc.sample_foreground_points(
                        labels, para_masks, line_masks, word_masks, line2para_idx
                    )
                )
            for b_i in range(len(inputs)):
                ...
                if args.hier_det:
                    ...
                    dict_input["point_labels"] = torch.ones(
                        (point_coords.shape[0], point_coords.shape[1]),
            ...
            with autocast():
                if args.hier_det:
                    (
                        ...
                        hi_iou_output,
                        word_masks_logits,
                    ) = model(batched_input, multimask_output=False)
                    loss_focal, loss_dice = loss_masks(
                        up_masks_logits, labels / 255.0, len(up_masks_logits)
                    )
                    loss_mse = loss_iou_mse(iou_output, up_masks, labels)
                    loss_lr = loss_focal * 20 + loss_dice + loss_mse

                    loss_focal_hr, loss_dice_hr = loss_masks(
                        hr_masks_logits, labels / 255.0, len(up_masks_logits)
                    )
            ...
            if misc.is_main_process():
                for ds_idx, (ds_name, ds_results) in enumerate(test_stats.items()):
                    ...
                    if iou_result > best_iou[ds_idx]:
                        ...
                        torch.save(
                            checkpoint, os.path.join(args.output, ds_name + "_best.pth")
                        )
    ...
    for idx_val, data_val in enumerate(data_loader):
        ...
        if (idx_val + 1) % 20 == 0:
            eta = datetime.timedelta(
                seconds=int(total_seconds_per_iter * (total - idx_val - 1))
            )
            print(
...
def evaluate(args, model, valid_dataloaders, valid_datasets_names):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        print("============================")
        results_k = inference_on_dataset(
            model, valid_dataloader, valid_dataset_name, evaluator, args
        )
