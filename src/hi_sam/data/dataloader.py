...
class ToTensor(object):
    def __call__(self, sample):
        ...
        if masks is not None:
            ...
            if sample.get("paragraph_masks", None) is not None:
                sample["paragraph_masks"] = torch.from_numpy(
                    sample["paragraph_masks"]
                ).permute(2, 0, 1)
                sample["word_masks"] = torch.from_numpy(sample["word_masks"]).permute(
                    2, 0, 1
                )
        return sample
...
class LargeScaleJitter(object):
    ...
    def __call__(self, sample):
        ...
        random_scale = (
            np.random.rand(1) * (self.aug_scale_max - self.aug_scale_min)
            + self.aug_scale_min
        )
        ...
        scaled_image = cv2.resize(
            image, dsize=scaled_size[::-1], interpolation=cv2.INTER_LINEAR
        )
        scaled_label = cv2.resize(
            label, dsize=scaled_size[::-1], interpolation=cv2.INTER_LINEAR
        )
        ...
        label = cv2.copyMakeBorder(
            scaled_label, 0, padding_h, 0, padding_w, cv2.BORDER_CONSTANT, value=0
        )
        sample.update(image=image, label=label, shape=torch.tensor(image.shape[:2]))

        if masks is not None:
            masks = cv2.resize(
                masks, dsize=scaled_size[::-1], interpolation=cv2.INTER_LINEAR
            )
            if len(masks.shape) < 3:
                masks = masks[:, :, np.newaxis]
            masks = masks[crop_y1:crop_y2, crop_x1:crop_x2, :]
            masks = cv2.copyMakeBorder(
                masks, 0, padding_h, 0, padding_w, cv2.BORDER_CONSTANT, value=0
            )
            if len(masks.shape) < 3:
                masks = masks[:, :, np.newaxis]
...
class ColorJitter(object):
    ...
    def __call__(self, sample):
        ...
        sample["image"] = (
            image.squeeze(dim=0).permute(1, 2, 0).numpy().astype(np.float32)
        )
        return sample
...
class RandomRotate(object):
    ...
    def apply(self, img, rm_img, bound_w, bound_h, interp=None, border_value=None):
        ...
        return cv2.warpAffine(
            img, rm_img, (bound_w, bound_h), flags=interp, borderValue=border_value
        )
    ...
    def __call__(self, sample):
        ...
        image = self.apply(
            image, rm_image, bound_w, bound_h, cv2.INTER_NEAREST, (128, 128, 128)
        )
        ...
        if masks is not None:
            ...
            if len(masks.shape) < 3:
                masks = masks[
                    :, :, np.newaxis
                ]  # to avoid channel disappearing when there is only one mask
            sample["line_masks"] = masks
            ...
            if masks is not None:
                ...
                masks = self.apply(
                    masks, rm_image, bound_w, bound_h, cv2.INTER_NEAREST, 0
                )
                if len(masks.shape) < 3:
                    masks = masks[:, :, np.newaxis]
                sample["paragraph_masks"] = masks
        ...
        if self.hier_det:
            ...
            if "HierText" in self.dataset_name:
                if line_num > 10:
                    ...
                    masks = [
                        get_one_mask(anns["line_masks"][l_idx], w, h)
                        for l_idx in select_line_idx
                    ]
                    masks = np.array(masks).transpose((1, 2, 0))
                    sample["line_masks"] = masks
                    masks = [
                        get_word_mask(anns["word_masks"][l_idx], w, h)
                        for l_idx in select_line_idx
                    ]
                    masks = np.array(masks).transpose((1, 2, 0))
                    sample["word_masks"] = masks
                    ...
                    line2para_index = [
                        line2para_index[l_idx] for l_idx in select_line_idx
                    ]
                    line2para_index_set = set(line2para_index)
                    masks = [
                        get_one_mask(anns["paragraph_masks"][l2p_idx], w, h)
                        for l2p_idx in line2para_index_set
                    ]
                    masks = np.array(masks).transpose((1, 2, 0))
                    sample["paragraph_masks"] = masks
                    new_l2p_index = []
                    ...
                    sample["line2paragraph_index"] = torch.tensor(
                        anns["line2paragraph_index"]
                    )
