...
EPSILON = 1e-5
TextEntityListType = Union[polygon_list.PolygonList, np_box_mask_list.BoxMaskList]
...
class HierTextEvaluator(object):
    ...
    def _init_new_accumulators(self):
        ...
        accumulators = {
            ...
            "num_prediction": 0.0,
            "num_groundtruth": 0.0,
            "tp_det_tightness_sum": 0.0,
        }
        # For E2E:
        if self._evaluate_text:
            accumulators_for_e2e = {
                "tp_e2e_cnt": 0.0,
                "tp_e2e_tightness_sum": 0.0,
            }
            accumulators.update(accumulators_for_e2e)
        return accumulators

    def _get_text(
        self, prediction_box_list: TextEntityListType, gt_box_list: TextEntityListType
    ):
        ...
        if self._evaluate_text:
            pred_text = [
                str(text) for text in prediction_box_list.get_field("pred_texts")
            ]
            gt_text = [str(text) for text in gt_box_list.get_field("gt_texts")]
    ...
    def _filter_dont_care_region(
        ...
        if invalid_gt_indices.size and prediction_box_list.num_boxes():
            ...
            valid_prediction_indices = np.where(
                np.max(precision, axis=1) < self._filter_invalid_thr
            )[0]
            prediction_box_list = self._gather(
                prediction_box_list, valid_prediction_indices
            )
    ...
    def _get_box_lists(self, eval_dict: NumpyDict):
        ...
        if self._text_box_type == TextBoxRep.MASK:
            ...
            prediction_box_list = polygon_list.PolygonList(eval_dict["detection_boxes"])
        else:
    ...
    def evaluate(self, accumulators):
        ...
        metrics["Det-Fscore"] = (
            2 * r_det * p_det / (r_det + p_det) if (r_det + p_det) else 0.0
        )
        metrics["Det-Tightness"] = (
            (accumulators["tp_det_tightness_sum"] / total_tp_det)
            if total_tp_det
            else 1.0
        )
        ...
        if self._evaluate_text:
            ...
            metrics["E2E-Fscore"] = (
                2 * r_e2e * p_e2e / (r_e2e + p_e2e) if (r_e2e + p_e2e) else 0.0
            )
            metrics["E2E-Tightness"] = (
                accumulators["tp_e2e_tightness_sum"] / total_tp_e2e
                if total_tp_e2e
                else 1.0
            )
