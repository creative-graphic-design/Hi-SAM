        ...
        for j in range(m):
            if intersection_aa_box[i, j] > 0:
                if area1[i] < EPSILON or area2[j] < EPSILON:
                    ...
                    intersection_area[i, j] = 0.0
                else:
                    intersection_area[i, j] = (
                        polygon_data1[i].intersection(polygon_data2[j]).area
                    )
            else:
                intersection_area[i, j] = 0.0
...
def iou(polygons1, polygons2):
    ...
    """
    intersection_area, area1, area2 = intersection_and_area(polygons1, polygons2)
...
def gather(polygons, indices, fields=None):
    ...
    """
    if indices.size:
        if np.amax(indices) >= polygons.num_boxes() or np.amin(indices) < 0:
