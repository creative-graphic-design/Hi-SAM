...
_E2E = flags.DEFINE_bool("e2e", False, "Whether to perform end-to-end evaluation.")
...
def draw_mask(vertices: np.ndarray, w: int, h: int, s: int = 1):
    ...
    return cv2.fillPoly(mask, [vertices], [1.0])[::s, ::s] > 0
...
def parse_annotation_dict(anno, eval_lines, eval_paragraphs, mask_stride):
    ...
    for paragraph in anno["paragraphs"]:
        ...
        if eval_paragraphs:
            if not gt_paragraph_mask or not paragraph["legible"]:
                gt_paragraph_mask = [
                    draw_mask(np.array(paragraph["vertices"]), w, h, mask_stride)
                ]
    ...
    for paragraph in anno["output_paragraphs"]:
        ...
        if eval_paragraphs:
            if not paragraph_mask:
                raise ValueError("Paragraph does not contain lines: %s" % paragraph)
    ...
    word_dict = {
        "gt_weights": (
            np.array(gt_word_weights) if num_gt_words else np.zeros((0,), np.float32)
        ),
        "gt_boxes": gt_word_polygons,
        "gt_texts": (np.array(gt_word_texts) if num_gt_words else np.zeros((0,), str)),
        "detection_boxes": word_polygons,
        "pred_texts": (np.array(word_texts) if num_pred_words else np.zeros((0,), str)),
    }
    ...
    if eval_lines:
        ...
        line_dict = {
            "gt_weights": (
                np.array(gt_line_weights)
                if num_gt_lines
                else np.zeros((0,), np.float32)
            ),
            "gt_masks": (
                np.stack(gt_line_masks, 0)
                if num_gt_lines
                else np.zeros((0, (h + 1) // 2, (w + 1) // 2), np.float32)
            ),
            "gt_boxes": (
                np.array(gt_line_boxes, np.float32)
                if num_gt_lines
                else np.zeros((0, 4), np.float32)
            ),
            "gt_texts": (
                np.array(gt_line_texts) if num_gt_lines else np.zeros((0,), str)
            ),
            "detection_boxes": (
            ...
            "detection_masks": (
                np.stack(line_masks, 0)
                if num_pred_lines
                else np.zeros((0, (h + 1) // 2, (w + 1) // 2), np.float32)
            ),
            "pred_texts": (
                np.array(line_texts) if num_pred_lines else np.zeros((0,), str)
            ),
        }
    ...
    if eval_paragraphs:
        ...
        paragraph_dict = {
            "gt_weights": (
                np.array(gt_paragraph_weights)
                if num_gt_paragraphs
                else np.zeros((0,), np.float32)
            ),
            "gt_masks": (
                np.stack(gt_paragraph_masks, 0)
                if num_gt_paragraphs
                else np.zeros((0, (h + 1) // 2, (w + 1) // 2), np.float32)
            ),
            "gt_boxes": (
            ...
            "detection_boxes": (
                ...
                if num_pred_paragraphs
                else np.zeros((0, 4), np.float32)
            ),
            "detection_masks": (
                np.stack(paragraph_masks, 0)
                if num_pred_paragraphs
                else np.zeros((0, (h + 1) // 2, (w + 1) // 2), np.float32)
            ),
        }
...
def main(argv: Sequence[str]) -> None:
    ...
    word_evaluator = evaluator.HierTextEvaluator(
        text_box_type=evaluator.TextBoxRep.POLY, evaluate_text=_E2E.value
    )
    if _EVAL_LINES.value:
        line_evaluator = evaluator.HierTextEvaluator(evaluate_text=_E2E.value)
    else:
        line_evaluator = None
